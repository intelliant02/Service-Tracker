...
<#-- ================ Subscreens ================ -->
<#macro "test">
 <p>"This is a test macro which is defined in DefaultScreenMacros.html.ftl"</p>
 </#macro>
...
<#if sri.doBoundaryComments()><!-- BEGIN form-single[@name=${.node["@name"]}] --></#if>
    ...
    <form name="${formId}" id="${formId}" class="validation-engine-init" method="post" action="${urlInfo.url}"<#if sri.isFormUpload(formNode["@name"])> enctype="multipart/form-data"</#if>>
        ...
        <!--===================== Example of Help sign =====================-->
         <#if formNode["@create-help"]! == "true">
         <#assign testScript>
         ...
         e.onmouseout = function() {
           document.getElementById('popup').style.display = 'none';
                 ...
                 <#t>${sri.appendToScriptWriter(testScript)}
                     <#assign formHelpValue = ec.resource.evaluateStringExpand(.node["@helpContent"], "")/>
                          <div id="parent" class="tiptext"  style="float: right; background:#ccc; border: 1px #333 solid; padding:5px; width:100px;">Help
                          <div id="popup" style="display: none; white-space: pre-wrap">${formHelpValue}</div>
                          </div>
                 </#if>
         </#if>
